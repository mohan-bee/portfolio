...
function PxIcon({ name, size = 18, title }) {
  if (name === "github") {
    return (
      <svg className="px-icon" width={size} height={size} viewBox="0 0 24 24" role={title ? "img" : "presentation"} aria-label={title}>
        <path d="M12 .5C5.65.5.5 5.65.5 12c0 5.09 3.29 9.39 7.86 10.91.58.11.79-.25.79-.56v-2.15c-3.2.7-3.87-1.36-3.87-1.36-.52-1.33-1.28-1.69-1.28-1.69-1.05-.72.08-.7.08-.7 1.16.08 1.77 1.19 1.77 1.19 1.03 1.76 2.7 1.25 3.36.96.1-.75.4-1.25.73-1.54-2.55-.29-5.24-1.28-5.24-5.69 0-1.26.45-2.28 1.19-3.09-.12-.29-.52-1.46.11-3.04 0 0 .98-.31 3.18 1.18A10.98 10.98 0 0 1 12 6.03c.98 0 1.96.13 2.88.39 2.2-1.49 3.17-1.18 3.17-1.18.64 1.58.24 2.75.12 3.04.74.81 1.18 1.83 1.18 3.09 0 4.42-2.69 5.39-5.25 5.68.42.36.79 1.07.79 2.16v3.14c0 .31.21.67.8.56A11.5 11.5 0 0 0 23.5 12C23.5 5.65 18.35.5 12 .5Z" />
      </svg>
    );
  }
...
function OpenSourcePanel() {
  return (
    <div className="open-source-panel" aria-hidden="true">
      <div className="open-source-orbit">
        <PxIcon name="github" size={78} />
        <span className="spark-drop spark-drop--one"><PxIcon name="spark" /></span>
        <span className="spark-drop spark-drop--two"><PxIcon name="spark" /></span>
        <span className="spark-drop spark-drop--three"><PxIcon name="spark" /></span>
      </div>
